...
function getActionErrorMessage(error, action) {
  ...
  if (errorMessage === "unsupported-page") {
    return "Firefox blocks extensions on this page. Try a normal website tab.";
  }
  ...
  if (errorMessage === "missing-storage-api") {
    return "Firefox did not expose local extension storage for this popup.";
  }

  if (errorMessage === "missing-scripting-api") {
    return "Firefox could not inject the page tools on this tab.";
  }

  if (errorMessage === "blocked-tab-access") {
    return "Firefox denied access to this tab. Try a normal http or https page.";
  }
